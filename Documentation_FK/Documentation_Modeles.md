# Pipeline and implementation in NH

# Data Loading|Processing

## Data Loading
- Data read by basin - Dyn., Stat. discharge (target)
- Class DataLoader loads the TS as pandas DataFrame, aligned by timestamps (column 'date' is csv)
- For now each timestamp have forcings, meteorological forecasts (if provided) and targets (all aligned)
- We have here raw aligned data


## Dataset Creation - Sequencing
- (File: neuralhydrology/datasetbaseloader.py)
- to create the DS we need:
  1. `seq_length`
  2. `predict_last_n`
  3. `horizon` - optional, for non AutoRegressive (AR) models

- slicing as follow:
  Input = [t-seq_length, ..., t-1]
  Targets = [t-predict_last_n, ..., t-1]

- Conclusion: no shifting here


## Batch creation:
- Combine multiple sequences to create batches of given size
- obtain batch Dict: 
  {  
    "x_d": [batch, seq_length, n_dyn_features],
      "x_s": [batch, n_static_features],
      "y": [batch, predict_last_n, n_targets],
  }
